defmodule ProfilePregnancyHealthTest do
  ...
  def setup_fake_cms(auth_token) do
    ...
    assert :ok =
             FakeCMS.add_images(wh_pid, [
               ...
               image_loading_1,
               image_loading_2,
               image_topics_for_you,
               image_baby_shower_cute,
    ...
    FakeCMS.add_img_to_page(
      ...
      0,
      image_topics_for_you.id
    )

    FakeCMS.add_img_to_page(wh_pid, "mnch_onboarding_loading_01", 0, image_loading_1.id)
    FakeCMS.add_img_to_page(wh_pid, "mnch_onboarding_loading_01_secondary", 0, image_loading_1.id)
    FakeCMS.add_img_to_page(wh_pid, "mnch_onboarding_loading_component_01", 0, image_loading_1.id)
    FakeCMS.add_img_to_page(wh_pid, "mnch_onboarding_loading_02_secondary", 0, image_loading_1.id)
  ...
  defp init_pregnancy_info(context) do
    ...
    |> FlowTester.set_contact_properties(%{
      "pregnancy_status" => "im_pregnant",
      "edd" => "24/04/2026",
      "pregnancy_sentiment" => "excited"
    })
  end
  ...
  describe "checkpoints" do
    test "pregnant mom 0%", %{flow: flow} do
      ...
      |> init_contact_fields()
      |> FlowTester.set_contact_properties(%{
        "checkpoint" => "pregnant_mom_profile",
      ...
      |> receive_message(%{
        text:
          "🟩🟩⬜⬜⬜⬜⬜⬜\r\n\r\nYour profile is already 25% complete! I think now is a good time to complete it, but it's up to you.\r\n\r\n👇🏽 What do you want to do next?" <>
            _,
        buttons:
          button_labels(["➡️ Complete profile", "View topics for you", "Explore health guide"])
      })
    end
    ...
    test "partner pregnant 0%", %{flow: flow} do
      ...
      |> init_contact_fields()
      |> FlowTester.set_contact_properties(%{
        "checkpoint" => "partner_of_pregnant_mom_profile",
        "profile_completion" => "0%"
      })
      |> FlowTester.start()
      |> receive_message(%{
    ...
    test "curious 0%", %{flow: flow} do
      ...
      |> init_contact_fields()
      |> FlowTester.set_contact_properties(%{
        "checkpoint" => "curious_pregnancy_profile",
        "profile_completion" => "0%"
      })
      |> FlowTester.start()
      |> receive_message(%{
    ...
    test "pregnancy_basic_info", %{flow: flow} do
      ...
      |> init_contact_fields()
      |> FlowTester.set_contact_properties(%{
        "checkpoint" => "pregnancy_basic_info",
        "profile_completion" => ""
      })
      |> FlowTester.start()
      |> Helpers.handle_basic_profile_flow()
    ...
    test "pregnancy_personal_info", %{flow: flow} do
      ...
      |> init_contact_fields()
      |> FlowTester.set_contact_properties(%{
        "checkpoint" => "pregnancy_personal_info",
        "profile_completion" => ""
      })
      |> FlowTester.start()
    ...
    test "pregnancy_daily_life_info", %{flow: flow} do
      ...
      |> init_contact_fields()
      |> FlowTester.set_contact_properties(%{
        "checkpoint" => "pregnancy_daily_life_info",
        "profile_completion" => ""
      })
      |> FlowTester.start()
      |> Helpers.handle_daily_life_flow()
  ...
  describe "profile pregnancy health - pregnant" do
    ...
    test "feelings then other 1st trimester (all)", %{flow: flow} do
      ...
      |> receive_message(%{
        text:
          "*It’s normal to have mixed feelings about your pregnancy*\r\n\r\nJust remember that you are strong and capable. The more information you have, the more you can prepare for what's coming next 🌟\r\n\r\nYour baby is growing quickly, already developing a brain, heart, facial features, and even feet, hands, fingernails, and toenails.\r\n\r\n👇🏽 Let's have a look at what you can expect.",
        buttons: button_labels(["Let's check it out"]),
        image:
          ...
          list:
            {"Choose a Topic",
             [
               ...
               {"item 3", "item 3"},
               {"item 4", "item 4"},
               {"Show me other topics", "Show me other topics"}
             ]}
        }
      ])
    end
    ...
    test "topics then dummy topic", %{flow: flow} do
      ...
      |> receive_message(%{
        ...
        buttons: [
          ...
          {"Choose another topic", "Choose another topic"}
        ]
      })
    end
    ...
    test "rate this article then not really", %{flow: flow} do
      ...
      |> receive_message(%{
        text:
          "Mmm maybe I need a bit more information about you...🤔\r\n\r\nWould you like to answer some more questions now?",
  ...
  describe "profile pregnancy health - partner pregnant" do
    test "question 1 - partner is pregnant", %{flow: flow} do
      ...
      |> contact_matches(%{
        "gender" => "",
        "pregnancy_status" => "partner_pregnant",
        "checkpoint" => "partner_of_pregnant_mom_profile",
        "profile_completion" => "0%"
      })
      |> receive_message(%{
    ...
    test "content intro then article topic", %{flow: flow} do
      ...
      |> receive_message(%{
        ...
        buttons:
          button_labels(["➡️ Complete profile", "Rate this article", "Choose another topic"])
      })
    end
    ...
    test "rate this article then yes not opted in then later", %{flow: flow} do
      ...
      |> receive_messages([
        %{
          text:
            "Thousands of people have signed up to receive these messages – they're the best way to stay in control 🙌🏾\r\n\r\nYou can always change your update choice in `Settings`"
        },
        %{
          text:
            "🟩🟩⬜⬜⬜⬜⬜⬜\r\n\r\nYour profile is already 25% complete!\r\n\r\n👇🏽 What do you want to do next?",
    ...
    test "i'm curious - question 1 then error then male", %{flow: flow} do
      ...
      |> receive_message(%{
        ...
        list:
          {"Other children",
           [
             {"@menu_items[0]", "No other children"},
             {"@menu_items[1]", "Yes, one"},
             {"@menu_items[2]", "Yes, two"},
             {"@menu_items[3]", "Yes, more than two"},
             {"@menu_items[4]", "Skip this question"}
           ]}
      })
    end
    ...
    test "i'm curious - question 2 then error then 0", %{flow: flow} do
      ...
      |> receive_message(%{
        ...
        list:
          {"Select option",
           [
             {"@menu_items[0]", "First trimester"},
             {"@menu_items[1]", "Second trimester"},
             {"@menu_items[2]", "Third trimester"},
             {"@menu_items[3]", "General pregnancy info"},
             {"@menu_items[4]", "Skip this question"}
           ]}
      })
    end
