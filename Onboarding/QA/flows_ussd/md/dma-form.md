...
card GetAssessment, then: CheckEnd do
  log("Fetching Form @config.items.assessment_tag")
  ...
  log("Starting Form @config.items.assessment_tag")
...
card CheckEnd when question_num == count(questions), then: End do
  ...
  log("Ending Form @config.items.assessment_tag")
...
card GetQuestion, then: DisplayQuestion do
  log("Getting question @question_num")
...
card DisplayQuestion when question_type == "multiselect_question",
...
card DisplayQuestion when question_type == "year_of_birth_question",
...
card DisplayQuestion when question_type == "freetext_question",
  then: QuestionExplainer do
...
card DisplayQuestion when question_type == "integer_question",
  then: QuestionExplainer do
...
card DisplayQuestion when question_type == "age_question",
  then: ValidateAge do
...
card DisplayQuestion, then: QuestionExplainer do
  log("Displaying question @question_num")
  question = questions[question_num]
  num_answers = count(question.answers)

  answers =
    reduce(
      with_index(question.answers),
      "",
      &"@concatenate(&2, &1[1] + 1, \". \", &1[0].answer, unichar(10))"
    )

  question_text = concatenate(question_text, "@unichar(10)", "@unichar(10)", answers)

  log("Question text: @question_text")
  question_response = ask("@question_text")

  assertion =
    isnumber(question_response) and question_response > 0 and
      question_response <= num_answers
end
...
card QuestionExplainer do
  log("Question type is @question_type")
  log("Your answer was @question_response to question number @question_num")
  then(ValidateInput)
end

card ValidateInput
     when assertion == false and question_type == "year_of_birth_question",
     then: QuestionError do
  log("Error input")
end

card ValidateInput
     when assertion == false and question_type == "integer_question",
     then: QuestionError do
  log("Error input")
end

card ValidateInput
     when assertion == false,
     then: QuestionError do
  log("Invalid input")
end
...
card QuestionError
     when question_type == "integer_question" and
...
card QuestionError
     when question_type == "year_of_birth_question" and
            lower("@question_response") != "skip",
     then: CheckEnd do
  log(
...
card QuestionResponse, then: StoreResponse do
  ...
  answer = question.answers[question_response - 1]
  question_id = questions[question_num].semantic_id
...
card DisplayEndPage do
  ...
  log("@page_id")
  message_body = response.body.body.text.message
  ask("@message_body")
  text("TODO: Back to Onboarding")
end
